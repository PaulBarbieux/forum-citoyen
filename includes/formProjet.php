...
<div class="form-group">
	...
	<input type="text" name="titre" class="form-control" placeholder="Titre pour identifier le projet" required value="<?php echo $values['titre'] ?>" <?php if (!$CONNECTED) echo 'disabled' ?>>
</div>
<div class="form-group">
	<label for="titre">Description</label>
	<textarea name="description" class="form-control" placeholder="Description ou explication du projet" rows="5" required <?php if (!$CONNECTED) echo 'disabled' ?>><?php echo $values['description'] ?></textarea>
</div>
<div class="form-group">
	<label for="titre">Attente</label>
	<textarea name="attente" class="form-control" placeholder="D&eacute;crivez votre attente par rapport &agrave; ce projet" rows="5" required <?php if (!$CONNECTED) echo 'disabled' ?>><?php echo $values['attente'] ?></textarea>
</div>
<div class="form-group">
	...
	<textarea name="ressources" class="form-control" rows="5" required <?php if (!$CONNECTED) echo 'disabled' ?>><?php echo $values['ressources'] ?></textarea>
</div>
<div class="form-group">
	...
	<textarea name="initiative" class="form-control" rows="5" required <?php if (!$CONNECTED) echo 'disabled' ?>><?php echo $values['initiative'] ?></textarea>
</div>
<div class="form-group">
	...
	<textarea name="demarches" class="form-control" rows="5" <?php if (!$CONNECTED) echo 'disabled' ?>><?php echo $values['demarches'] ?></textarea>
</div>